...
\usepackage{lmodern}
\usepackage[letterpaper,margin=1in,headheight=14pt,headsep=18pt]{geometry}
...
\begin{document}
\begin{abstract}
We prove that the error terms in Gauss's circle problem and Dirichlet's divisor problem are
$O_\eps(X^{573/1828+\eps})$.
The first-spacing frequencies $(l,kl,l\sqrt{k})$ lie on a quadratic cone.
We organize their fourth moments by pairs: differences along one generator carry the radial
additive energy, while differences between generators retain two distinct angular widths.
An elementary factorization count gives
$K(W^2L^2+VWL)(KL)^\eps$ for the latter configurations.
A projected difference-energy identity makes this estimate uniform in complex coefficients
and in the third physical coordinate; a periodization argument transfers it to the actual
localized averages without replacing approximate relations by exact ones.
Combining these algebraic fourth moments with the amplitude-dependent wave-envelope theorem
of Guth and Maldague gives square-root first spacing for $4<q\le9/2$ when
$K^{q-4}\le L^{6-q}$ and $L^{3q-8}\le K^{8-q}$.
The arithmetic input and block lengths are those of Li and Yang.
All construction conditions are verified on the difficult range, and four fixed moments suffice
