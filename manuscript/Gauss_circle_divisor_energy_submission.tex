...
\fancyhead[C]{\ifodd\value{page}{\small XIYU HU}\else{\small FIRST SPACING IN THE CIRCLE AND DIVISOR PROBLEMS}\fi}
\fancyfoot[C]{\thepage}
...
\hypersetup{pdftitle={A remark on first spacing in Gauss's circle problem and Dirichlet's divisor problem},pdfauthor={Xiyu Hu},pdfsubject={Algebraic pair energies on a ruled cone and amplitude-dependent first spacing},pdfkeywords={circle problem, divisor problem, first spacing, additive energy, ruled cone}}
\title[First spacing in the circle and divisor problems]{A remark on first spacing in Gauss's circle problem\texorpdfstring{\\}{ }and Dirichlet's divisor problem}
\author{Xiyu Hu}
...
cutoff in the divisor problem. The main idea is a refined use of Guth and
Maldague's amplitude-dependent wave-envelope theorem in the first-spacing
analysis, retaining the amplitude information throughout the argument.
\end{abstract}
\maketitle
...
For numerical comparison, the exponent $573/1828=0.3134573304\ldots$ is also
smaller than $517/1648=0.3137135922\ldots$, the exponent stated by Bourgain
and Watt in~\cite[Theorem~2]{BW17}.
...
Here \cite{BW17} is used to identify the construction hypotheses behind the arithmetic interface restated in \cite{LY}. The proof does not rely on its first-spacing estimates or numerical conclusions.
...
The author is grateful to Xiaochun Li for suggesting the use of Jing and Wu's
discrete formulation~\cite{JW}, which led to a simpler and clearer presentation
of the first-spacing argument.
